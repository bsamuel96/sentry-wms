...
| SO edit events (v1.23.0) | `PUT /api/admin/sales-orders/<so_id>` | cookie + CSRF | emits `salesorderedit.completed/1` carrying the per-field diff; a status edit to SHIPPED additionally fires `ship.confirmed/1` (carrier derived from Ship Method via `carrier_from_ship_method`, empty `tracking_numbers` allowed for local-pickup orders) |
...
| Operations Dashboard (v1.23.0) | `GET /api/v1/dashboard/received?start=&end=&warehouse_id=`, `GET /api/v1/dashboard/shipping-health?start=&end=&warehouse_id=` | cookie + CSRF (logged-in admin session) | `received` aggregates `audit_log` RECEIVE rows per PO; `shipping-health` returns per-channel outbound health for the admin-defined channel set in `app_settings.dashboard_bubble_origins` (a JSON array of `{origin, label}` matched against `sales_orders.order_origin`; empty -> no channels). start/end default to today |
...
| POS endpoint surface (v1.10.0) | `GET /api/v1/pos/availability`, `POST /api/v1/pos/validate-cart`, `POST /api/v1/pos/checkout`, `POST /api/v1/pos/refund` | Either `X-WMS-Token` with `pos.dispatch` scope, or the mobile user's bearer JWT when the user is ADMIN / has the `sell` grant. JWT access is limited to the user's assigned warehouses. Token callers keep the fourth-direction isolation contract; mixed-direction tokens are rejected 403 `cross_direction_scope_violation`. | Per-route idempotency on `sales_orders.idempotency_key` (UNIQUE) with SHA-256 body hash + `cached_response_body` for exact-bytes replay; `X-Idempotent-Replay: true` on cache hits; same key + different body returns 409. Atomic checkout + refund via per-line `SELECT ... FOR UPDATE` ordered by `(item_id, bin_id)` to prevent deadlock; `SET LOCAL lock_timeout` / `statement_timeout` translates contention to 503 `lock_contention` with `Retry-After: 1`. Refund enforces a 90-day window, card-vs-cash tender lock, and once-per-original-SO guard. PCI-scope guard at the Pydantic boundary: card tenders are an explicit allowlist (`card_brand`, `card_last4`, `auth_code`, `external_ref`); any other field (PAN-shaped or otherwise) fails 422. |
| Returns / RMA (v1.24.0) | `POST /api/admin/sales-orders/<so_id>/create-rma`, `POST /api/admin/sales-orders/<so_id>/receive-return` (admin); `GET /api/v1/pos/sales-orders/<so_number>`, `POST /api/v1/pos/reference-orders` (`X-WMS-Token` `pos.dispatch`) | cookie + CSRF (sales-orders page key) on admin; token on POS | Post-fulfillment order types (`replacement` / `exchange` / `return`) link to their original via `parent_so_id` with `<orig>-RMA` / `-REPLACEMENT` / `-EXCHANGE` / `-REFUND` numbering (migration 070-073). `create-rma` mints the goods-in return SO; `receive-return` books a line into a disposition bin (idempotent on a handheld-supplied key), advances the RMA status, and emits `return.received/1`. POS checkout mints replacement/exchange children off an attached original; `reference-orders` stubs a source-system-only original so a child can link it. |
...
```json
{
  ...
  "user": {
    ...
    "warehouse_id": 1,
    "warehouse_ids": [1],
    "allowed_functions": ["receive", "putaway", "pick", "pack", "ship", "count", "transfer", "sell"],
    "is_active": true
  }
}
```
